...
def calculate_theoretical_factors(ticker_symbol, name_map, market_returns):
    ...
    新增：AI 決策建議 (Buy/Wait) 與 動態位階買點 (邏輯修正版)
    """
    try:
        ...
        # --- 6. & 7. 【改良版】AI 決策與動態買點同步邏輯 ---
        # 修正說明：解決「建議買點」與「AI決策」不一致的問題。
        # 邏輯：若 AI 叫進，買點即為現價；若 AI 叫等，買點才顯示下方支撐。

        bias_ma20 = (current_price - ma20) / ma20 # 月線乖離率
        ...
        if score < 60:
            ai_advice = "😐 觀望"
            buy_suggestion = ma20 # 僅供參考的支撐
            buy_note = "評分不足(風險高)"
            
        # B. 高分優選區 (Score >= 60) -> 進入戰術決策
        else:
            # --- 狀況 1: 多頭強勢股 (股價 > 月線) ---
            if current_price > ma20:
                # 1-1. 乖離率過大 (>10%)：過熱
                if bias_ma20 > 0.1:
                    target_support = ma5 # 強勢股回檔看5日線
                    ...
                    if gap < 0.02: # 距離5日線很近
                        ...
                        ai_advice = f"📉 等回檔 ({round(gap*100, 1)}%)"
                        buy_suggestion = target_support
                        buy_note = "乖離過大/等MA5"
                
                # 1-2. 趨勢穩健 (乖離 3% ~ 10%)：最舒服的波段位置
                elif bias_ma20 > 0.03:
                    target_support = ma10 # 穩健股看10日線
                    gap = (current_price - target_support) / current_price
            ...
            # --- 狀況 2: 整理或修正股 (股價 < 月線) ---
                ...
                if current_price > ma60:
                    ...
                    # 連季線都跌破，看是否超跌
                    if bias_ma20 < -0.05: # 負乖離過大
                        ai_advice = "⚡ 反彈搶進"
                        buy_suggestion = current_price
                        buy_note = "乖離過大(搶反彈)"
                    else:
                        ai_advice = "⚠️ 趨勢轉弱"
                        buy_suggestion = current_price * 0.9 # 需更保守
                        buy_note = "需觀察打底"

        # 若評分極高 (>=80) 且建議為買進，升級為強力買進
        if score >= 80 and "買進" in ai_advice:
            ai_advice = "🚀 強力買進"
